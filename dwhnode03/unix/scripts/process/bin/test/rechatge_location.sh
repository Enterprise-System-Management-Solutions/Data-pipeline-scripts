...
FROM
    ...
    FROM
        (select/*+ parallel (A.16)*/ MSISDN,M09_LOCATION,TO_NUMBER(TRIM(LEADING '-' FROM CHARGINGTIME)) as CHARGINGTIME
        from
            (
            ...
            FROM
                ...
                (SELECT /*+ parallel (A.16)*/  MSISDN,RE30_ENTRY_HOUR
                FROM L3_RECHARGE A
        ...
        union all
        select /*+ parallel (B.16)*/  MSISDN,M09_LOCATION,TO_NUMBER(TRIM(LEADING '-' FROM CHARGINGTIME)) as CHARGINGTIME
        from
            (
            SELECT /*+ parallel (A.16)*/  A.MSISDN,A.M09_LOCATION,B.RE30_ENTRY_HOUR-A.M07_ANSWER_HOUR AS CHARGINGTIME
            FROM
                (SELECT /*+ parallel (A.16)*/ M04_MSISDNAPARTY AS MSISDN,M09_LOCATION ,SUBSTR((M07_ANSWERTIMESTAMP),1,8) AS M07_ANSWERTIMESTAMP ,SUBSTR(M07_ANSWERTIMESTAMP,9,4) AS M07_ANSWER_HOUR
                FROM L1_MSC A
                WHERE SUBSTR((M07_ANSWERTIMESTAMP),1,8)=(SELECT TO_CHAR(DATE_VALUE,'RRRRMMDD') AS DATE_VALUE FROM DATE_DIM WHERE TO_CHAR(DATE_VALUE,'RRRRMMDD')=TO_CHAR(SYSDATE-1,'RRRRMMDD'))
