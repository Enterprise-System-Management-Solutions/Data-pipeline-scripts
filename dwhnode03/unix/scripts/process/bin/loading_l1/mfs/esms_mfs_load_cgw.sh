...
SET verify off
SET linesize 400
SELECT '/data02/mfs/cgw/process_dir'||','||'/data02/mfs/cgw/process_dir'||file_name||','||file_name||','||file_id
FROM cdr_head
 WHERE source='cgw'
   AND process_status=30
   and rownum < 5
...
INSERT INTO L1_CGW
SELECT TO_DATE(TO_CHAR(SYSDATE,'MM/DD/YYYY'),'MM/DD/YYYY'),'$1',ID,GLOBAL_ID,TID,USER_ID,USERNAME,OPERATION,MSISDN,PREPAID,RESULT_CODE,RESULT_DESC,CURRENT_BALANCE,PREVIOUS_BALANCE,AMOUNT,TRANSACTION_TIME
FROM CGW_EXT
/
COMMIT
/
...
lock=/data02/scripts/process/bin/l1_cgw_lock  export lock
...
cd /data02/scripts/process/bin/loading_l1/mfs
sh esms_create_mfs_ext_cgw.sh $v3
createSMSCUpdate $v3
insertSMSCUpdate $v3
