...
LIMIT=100
MINCOUNT=0
LDIR=/data02/sftp_msc/huawei/merge_dir
MDIR=/data02/sftp_msc/huawei/dump_dir

getFileList()
{

cd $LDIR
filecount=`ls huawei_msc*.csv | wc -l`
if [ "$filecount" -gt "$MINCOUNT" ] ; then
        ls  huawei_msc*.csv | head -$LIMIT
fi
...
lock=/data02/scripts/process/bin/file_reg_msc_merge_huawei_lock  export lock
