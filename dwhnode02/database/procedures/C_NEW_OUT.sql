...
-- C_NEW_OUT  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.c_new_out is
...
insert into c_narayanganj_new_out
select vdate_key,msisdn from
(select msisdn
from
(
...
where msisdn in (select msisdn from c_narayanganj_out_with_idle where date_key=vdate_key);
commit;
end;
/
