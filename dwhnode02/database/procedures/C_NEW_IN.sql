...
-- C_NEW_IN  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.c_new_in is
...
begin
    ...
    insert into  c_narayanganj_new_in 
    select vdate_key,a.msisdn from
    (select msisdn
    from
    (
    ...
    select v372_callingpartynumber as msisdn,v387_chargingtime_key 
        ...
        where v387_chargingtime_key = (select a.date_key from date_dim a where a.date_value = to_date (sysdate-1,'dd/mm/rrrr'))-- and to_date ('17/04/2020','dd/mm/rrrr'))
    ...
    group by msisdn, v387_chargingtime_key
    )
    group by msisdn) a
    where msisdn not in (select msisdn from c_narayanganj_every_day_active);
    commit;
end;
/
