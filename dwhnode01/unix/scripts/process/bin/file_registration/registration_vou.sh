...
SOURCETIME=cbs_cdr_vou_2019121712
cd $LDIR
...
values(FILE_ID.nextval,'$1',30,SYSDATE,'vou')
/
COMMIT
/
EXIT
...
f1=`ps -ef|grep 'registration_vou.sh'|grep -v grep|wc -l`
if [ $f1 -eq 2 ]
then
...
for fil in $fileList
do
echo "moving from /data02/cbs_cdrs/vou/  to /data02/cbs_cdrs/vou/process_dir for ${fil}"
mv $LDIR/$fil /data02/cbs_cdrs/vou/process_dir/$fil
