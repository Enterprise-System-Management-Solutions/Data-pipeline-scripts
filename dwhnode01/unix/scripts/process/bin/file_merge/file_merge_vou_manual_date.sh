...
lock=/data02/scripts/process/bin/cbs_vou_file_merge_$date  export lock
...
FILES=`ls -ltr cbs_cdr_vou_*${date}*.add | awk -F" " {'print $9'}`
for i in $FILES
do
...
nFILES=`ls -ltr new*${date}*.add | awk -F" " {'print $9'}`
for i in $nFILES
do
