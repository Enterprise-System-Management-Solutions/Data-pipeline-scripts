...
SET verify off
SET linesize 400
SELECT '/data02/nokia/process_dir'||','||'//data02/nokia/process_dir'||file_name||','||file_name||','||file_id
FROM cdr_head
 WHERE source='nokia'
   AND process_status=30
   and rownum < 1001 
...
lock=/data02/scripts/process/bin/l1_nokia  export lock
