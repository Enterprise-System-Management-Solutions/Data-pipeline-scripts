...
SET verify off
SET linesize 400
SELECT '/data02/mfs/ussd_20200604/process_dir'||','||'/data02/mfs/ussd_20200604/process_dir'||file_name||','||file_name||','||file_id
FROM cdr_head
 WHERE source='ussd_20200604'
   AND process_status=30
   and rownum < 100 
/
EOF
...
sh esms_create_mfs_ext_ussd_20200604.sh $v3
createSMSCUpdate $v3
insertSMSCUpdate $v3
