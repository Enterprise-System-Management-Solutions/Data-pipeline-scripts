--
-- P_L1_TO_L2_VOICE  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.P_L1_TO_L2_VOICE (P_PROCESS_DATE VARCHAR2) IS
    VCOUNT NUMBER;
    VDATE_KEY NUMBER;
    VSTATUS NUMBER;
...
BEGIN
  ...
  IF VCOUNT >= 1  THEN
    ...
    IF VSTATUS = 96 THEN
        ...
        WHERE TO_DATE(V.PROCESSED_DATE,'DD/MM/RRRR') = VDATE
        AND TO_CHAR(B.DATE_VALUE,'YYYYMMDD')=TO_CHAR(TO_DATE( SUBSTR(V.V387_CHARGINGTIME,1,12), 'YYYYMMDDHH24MI')+6/24, 'YYYYMMDD')--WHERE TO_DATE(B.DATE_VALUE,'DD/MM/YYYY')=TO_DATE(TO_DATE( SUBSTR(V.V387_CHARGINGTIME,1,8), 'YYYYMMDD'), 'DD/MM/YYYY')
