...
INSERT INTO L1_HUAWEIUDN(PROCESSED_DATE, PROCESSED_HOUR, FILE_ID, IP1_STARTTIME, IP2_ENDTIME, IP3_MS_IP, IP4_MS_PORT, IP5_SERVER_IP, IP6_SERVER_PORT, IP7_MSISDN, IP8_IMSI, IP9_IMEI, IP10_CGI, IP11_SAI, IP12_ECGI, IP23_UPLINK_TRAFFIC, IP24_DOWNLINK_TRAFFIC, IP14_RAT_TYPE, IP19_RAI, IP20_TAI, IP22_LAI, IP37_SAC, IP38_LAC)
(SELECT TO_DATE(TO_CHAR(SYSDATE,'MM/DD/YYYY'),'MM/DD/YYYY'), TO_CHAR(SYSDATE, 'HH24') AS PROCESSED_HOUR, '$2', STARTTIME, ENDTIME, MS_IP, MS_PORT, SERVER_IP, SERVER_PORT, MSISDN, IMSI, IMEISV, CGI, SAI, ECGI, UPLINK_TRAFFIC, DOWNLINK_TRAFFIC,RAT_TYPE,RAI,TAI,LAI,SAC,LAC  FROM EXT_HUAWEIUDN )
/
COMMIT
/
UPDATE cdr_head
...
lock=/data02/scripts/dwh/lock/HuaweiUDN_load_lock  export lock
