...
SELECT '/data04/ipdr_u2000/alarm/process_dir'||','||'/data04/ipdr_u2000/alarm/process_dir'||file_name||','||file_name||','||file_id
 ...
 WHERE source='alarm'
   AND process_status=30
   and file_name like '%'||(SELECT TO_CHAR(TRUNC(TO_DATE('$dt','RRRR/MM/DD')),'rrrrmmdd') from dual)||'%'
   and rownum <200
/
EOF
}
...
(select TRUNC(TO_DATE($2,'RRRR/MM/DD')),'$1',OBJECT_IDENTITY_NAME, OBJECT_IDENTITY, PRODUCT_NAME, NE_TYPE, NE_OBJECT_IDENTITY, ALARM_SOURCE, ALARM_NAME, TYPE, OCCURRENCE_TIME, CLEARANCE_TIME, LOCATION_INFORMATION, ALARM_ID, OBJECT, INSTANCE_TYPE, ADDTIONAL_TEXT FROM ALARM_EXT)
/
COMMIT
/
UPDATE cdr_head
